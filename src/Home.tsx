...
export const Home: FC<HomeProps> = ({
  ...
  useEffect(() => setTitle(AppTitle), []);

  const nav = useNavigate();

  return (
    <div className="flex flex-col gap-3">
      <button className="btn btn-lg btn-primary" onClick={() => nav("/setup")}>
        Play
      </button>
      ...
      <div className="card bg-base-100 shadow-xl">
        <div className="card-body p-3">
          <h2 className="card-title">Avg Game Time</h2>
          {avgGameDurationsByPlayerCount.length > 0 ? (
            ...
            <p>Play a game to see the average game times!</p>
          )}
        </div>
      </div>
      <div className="card bg-base-100 shadow-xl">
        <div className="card-body p-3">
          <h2 className="card-title">Chart Explanation</h2>
          <h1 className="card-title mt- flex justify-center items-center h-full w-full">
            {playerForChart}
          </h1>
        </div>

        <div className="card-title mt-4 flex justify-center items-center h-full w-full">
          <PieChart
